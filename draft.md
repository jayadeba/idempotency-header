...
The server MAY enforce a time based idempotency keys, thus, be able to purge or delete a key upon its expiry. Resource SHOULD publish expiration policy related documentation.

## Idempotency Fingerprint

An idempotency fingerprint MAY be used in conjunction with with an idempotency key to detemine the uniqueness of a request and is generated from request payload data. An idempotency fingerprint is generated by the server implementation and MAY employ any of the following patterns.
...
## Responsibilities
    Client
      For idempotent resource endpoints, the client MUST present a unique idempotency key in the request.
      
    Server
      Generate Idempotency Fingerprint when required.
      Enforce compliance with the Idempotency enforcement scenarios.
      Manage the lifecycle of the Idempotency Key.
      Document the client-side idempotency contact in the API documentation.
